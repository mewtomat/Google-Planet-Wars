...
BOTENEMY="$BOT8"
	...
	echo "Match= $BOT1 vs $BOTENEMY"
	for i in `seq 1 100`;
	do
		echo "match $i"
		echo `java -jar tools/PlayGame.jar maps/map$i.txt 1000 200 log.txt "$BOT1" "$BOTENEMY"` > logsript
		#echo `./playgame maps/map$i.txt 1000 200 log.txt "$BOT1" "$BOTENEMY"` > logsript
	done
# echo 'exit'
